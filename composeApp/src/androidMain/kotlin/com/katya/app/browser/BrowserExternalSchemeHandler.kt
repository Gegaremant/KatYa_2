...
object BrowserExternalSchemeHandler {
    ...
    fun handle(context: Context, uri: Uri?): Boolean {
        ...
        return when (scheme) {
            ...
            in EXTERNAL_VIEW_SCHEMES -> handleViewIntent(context, uri)
                ...
                println(TAG + ": " + "blocked unknown scheme: $scheme (uri=$uri)")
                Toast.makeText(
                    context,
    ...
    private fun handleIntentScheme(context: Context, url: String): Boolean {
        val intent = runCatching {
            ...
            println(TAG + ": " + "parseUri failed for $url: ${it.message}")
            return true
        }
        ...
        try {
            ...
            println(TAG + ": " + "No app handles $url; fallback=${fallback != null}")
        } catch (e: Exception) {
            println(TAG + ": " + "startActivity failed for $url: ${e.message}")
        }
        ...
        if (!fallback.isNullOrBlank()) {
            try {
                ...
                println(TAG + ": " + "fallback $fallback failed: ${e.message}")
            }
        }
    ...
    private fun handleViewIntent(context: Context, uri: Uri): Boolean {
        ...
        try {
            ...
            println(TAG + ": " + "ACTION_VIEW failed for $uri: ${e.message}")
        }
        return true
    }
}
