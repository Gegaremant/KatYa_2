...
import com.katya.app.data.currentConversationIdOrNull
...
import katya.composeapp.generated.resources.tool_execute_shell_command_description
import katya.composeapp.generated.resources.tool_execute_shell_command_name
import org.koin.java.KoinJavaComponent.inject

private const val TOOL_DESCRIPTION = """Execute a shell command in the Linux sandbox (Debian by default, Termux optionally) and return stdout, stderr, exit code, and current working directory. The environment runs via proot inside the app.
...
Password-only servers (no key auth): this shell can't answer interactive password prompts directly (no PTY, ssh reads from /dev/tty). Heredoc stdin will NOT deliver a password. Install sshpass once (`apt-get install -y sshpass` on Debian, `apk add sshpass` on Termux), then drive the connection as `sshpass -p '<password>' ssh <alias> '<remote-cmd>'` — or `sshpass -f <password-file> ssh <alias>` to keep the password off the command line. sshpass fakes a PTY internally, which is the only path that actually works.
...
Install extra packages with: `apt-get install -y <package>` (Debian) or `apk add <package>` (Termux)
