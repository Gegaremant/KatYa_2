 ...
 * `~/skills/<id>/`. The filesystem is the single source of truth — this
 ...
 * knows what's available at `~/skills/<id>/`; their contents already live in
 * Termux and are read there via `execute_shell_command`.
 *
 * [isBuiltIn] marks skills that ship inside the app (loaded from compose resources
 * rather than Termux). They are always available when the sandbox is up,
...
/** Where a skill is downloaded from when installing into Termux. */
...
/**
 * The raw files of a skill downloaded from GitHub, ready to be written into Termux
 * at `~/skills/<id>/`. [rawSkillMd] is stored verbatim as `SKILL.md`;
