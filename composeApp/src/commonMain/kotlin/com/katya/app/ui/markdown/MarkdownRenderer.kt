...
import com.katya.app.ui.markdown.math.MathFormula
 ...
 *
 * Kai-UI blocks dispatch to [KatyaUiRenderer]; pass `isInteractive = false` to render them as
...
fun MarkdownContent(
    ...
    val doc = remember(content) {
        runCatching { parseMarkdown(content) }.getOrElse {
            MarkdownDocument(persistentListOf(Paragraph(persistentListOf(com.katya.app.ui.markdown.Text(content)))))
        }
    }
...
private fun BlockRenderer(
    ...
    when (block) {
        ...
        is KatyaUiBlock -> KatyaUiRenderer(
            node = block.node,
        ...
        is KatyaUiError -> CodeFenceBlock(
            language = "json",
            code = block.rawJson,
            modifier = Modifier.padding(vertical = 4.dp),
        )
