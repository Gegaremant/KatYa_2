...
- В `native-x86_64.zip` **не было `libxray.so`** — только proot. Туннель запускать было нечем, и на этой архитектуре он не мог подключиться в принципе. Бинарник добавлен (официальная сборка Xray-core для android/amd64).
- Для `armeabi-v7a` официальной сборки xray не существует, и её нельзя выдумать. Вместо молчаливого отказа приложение теперь **просит root не для того, чего не может сделать**: проверка бинарника идёт до запроса прав, и в карточке появляется прямое объяснение — для этой архитектуры нет бинарника, скачай компонент заново или пользуйся обычным подключением.
- Это и есть причина из полевого отчёта «запрашивает root и не подключается»: права давали, а запускать было нечего.
...
- Подпись: APK v3.1.2 и этот релиз — один сертификат (CN=KatYa, SHA-256 `79FF:04:D2:…:2C:70`).
- Sandbox, VLESS, DeepSeek, God Mode, цвета и обновление поверх установки — это требует прогона на телефоне; логи приложения приложены к полевым тестам.
